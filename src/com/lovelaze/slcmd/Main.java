...
public class Main {
	...
	private void createOptions(Options options) {
		...
		Option w = OptionBuilder.withArgName("-w").hasArg()
				.withDescription("Time window for a departure")
				.create("w");

		Option t = OptionBuilder.hasArgs(2).withArgName("-t")
	...
	public void printDepartures(String station, int timeWindow)
		...
		for (int i = 0; i < departures.size(); i++) {
			if (departures.get(i).size() > 0) {
				System.out.println("-----"
						+ departures.get(i).get(0).getTransport().toUpperCase()
						+ "-----");
			}

			for (Departure d : departures.get(i)) {
				String temp = "\t" + d.getTime() 
						+ "\t- " + d.getLine()
						+ " " + d.getDestination();
	...
	public void printTrips(String origin, String destination) throws Exception {
		...
		for (Trip t : trips) {
			System.out.println(t.getOrigin() + " " + t.getDepartureTime()
					+ " -> " + t.getDestination() + " " + t.getArrivalTime()
					+ " [" + t.getDuration() + "]");
			ArrayList<SubTrip> subs = t.getSubTrips();
			for (SubTrip s : subs) {
				System.out.println("\t(" + s.getTransportType() + " "+ s.getTransportLine() + " " + s.getTransportTowards()
						+ "): " + s.getOrigin() + " " + s.getDepartureTime()
						+ " -> " + s.getDestination() + " "+ s.getArrivalTime());
			}
		}
